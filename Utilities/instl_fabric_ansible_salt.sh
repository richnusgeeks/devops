#! /usr/bin/env bash
############################################################################
# File name : instl_fabric_ansible_saltstack.sh
# Purpose   : Install Fabric, Ansible, Saltstack on CentOS/RHEL 6.x .
# Usages    : ./instl-remote-control.sh <-i|--install|-r|--clean|-d|--dump>
...
prntUsage() {
    ...
    "$ECHO" "        -i|--install Install Fabric/Ansible/Salt,"
    "$ECHO" "        -r|--clean   Cleanup Fabric/Ansible/Salt,"
    "$ECHO" "        -d|--dump    Dump various Fabric/Ansible/Salt related info,"
    exit 0
