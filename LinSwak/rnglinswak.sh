#! /usr/bin/env bash
############################################################################
# File name : rnglinswak.sh
# Purpose   : RichNusGeeks SWAK for all RHEL 6.x (x>=3) based GNU/Linux roles.
# Usages    : ./rnglinswak.sh [-r|--rds|-c|--mcb|-p|--pgr|-g|--pgp|-r|--msg
#                                |-h|--hap|-t|--trc]
...
prntUsage() {
    ...
    "$ECHO" "                |-h|--hap|-t|--trc]"
    "$ECHO" "        -r|--rds Verify for Redis role,"
    "$ECHO" "        -c|--mcb Verify for Couchbase role,"
    "$ECHO" "        -p|--pgr Verify for Postgres role,"
    "$ECHO" "        -g|--pgp Verify for PGPool role,"
    "$ECHO" "        -r|--msg Verify for RabbitMQ role,"
